...
describe('Dashboard Handler', () => {
	...
	it('GET /workflows/:id/timeline returns timeline-shaped data', async () => {
		...
		const body = (await res.json()) as {
			id: string;
			timeline: Array<{ name: string; startedAt: number; duration: number; status: string }>;
		};
		expect(body.id).toBe('dash-echo-1');
...
export function WorkflowDetail() {
	...
	useEffect(() => {
		Promise.all([api.getWorkflow(workflowId), api.getTimeline(workflowId)]).then(([wf, tl]) => {
			setWorkflow(wf);
			setTimeline(tl);
			setLoading(false);
		});
		...
		<div className="space-y-6">
			<div>
				...
				<p className="text-sm text-zinc-400">
					{workflow.type} &middot; {workflow.status}
				</p>
			</div>
				...
				<div className="mt-3 space-y-2 text-xs">
					...
					{step.result != null && (
						<div>
							...
							<pre className="bg-zinc-900 rounded p-2 text-zinc-400 whitespace-pre-wrap overflow-x-auto">
