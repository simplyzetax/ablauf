...
```ts
create<T extends BaseWorkflow>(
  workflow: WorkflowConstructor<T>,
  options: {
    id?: string;
    payload: InferInput<T>;
  }
): WorkflowStub<T>
```
...
A typed `WorkflowStub` for interacting with the workflow instance.
...
```ts
sendEvent<T extends BaseWorkflow>(
  workflow: WorkflowConstructor<T>,
  options: {
    id: string;
    event: InferEventName<T>;
    payload: InferEventPayload<T, InferEventName<T>>;
  }
): Promise<void>
```
...
// Typed
status<T extends BaseWorkflow>(
  id: string,
  workflow: WorkflowConstructor<T>
): Promise<WorkflowStatusResponseFor<T>>
```
...
```ts
type WorkflowStatusResponseFor<T extends BaseWorkflow> = {
  id: string;
  type: string;
  status: WorkflowStatus;
  payload: InferInput<T>;
  result: InferResult<T> | null;
  error: string | null;
  steps: StepInfo[];
  createdAt: number;
  updatedAt: number;
