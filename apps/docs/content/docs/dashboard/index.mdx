...
- **Workflow Table** — Full-width data table with status indicators, type badges, and relative timestamps. Filter by status and type, or search by workflow ID with the `/` keyboard shortcut
- **Detail Panel** — Slide-over sheet (65% viewport) for inspecting payload, result, errors, and step execution for any workflow
...
```bash
bunx @der-ablauf/dashboard --url https://my-worker.example.com --port 3000
...
// ablauf.list() will throw ObservabilityReadNotConfiguredError
```
...
See the [Server docs](/docs/server#custom-observability-provider) for the full provider interface and examples.
