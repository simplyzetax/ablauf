...
<Callout type="info">
	Most users should start with the [functional API](/docs/workflows/functional) for less boilerplate. Use classes when you need additional structure or methods.
</Callout>
...
A Zod schema for SSE message types your workflow can broadcast.

```ts
static sseUpdates = z.discriminatedUnion("type", [
...
const sseSchema = z.discriminatedUnion("type", [
	z.object({ type: z.literal("started"), url: z.string() }),
	z.object({ type: z.literal("progress"), percent: z.number() }),
	z.object({ type: z.literal("complete"), outputUrl: z.string() }),
]);
type SSEUpdates = z.infer<typeof sseSchema>;
...
export class DocumentProcessingWorkflow extends BaseWorkflow<
	Payload,
	Result,
	Events,
	SSEUpdates
> {
	...
	async run(
		...
		sse.broadcast({ type: "started", url: payload.fileUrl });
		...
		sse.broadcast({ type: "progress", percent: 33 });
		...
		sse.broadcast({ type: "progress", percent: 66 });
		...
		sse.broadcast({ type: "complete", outputUrl: uploaded.url });
