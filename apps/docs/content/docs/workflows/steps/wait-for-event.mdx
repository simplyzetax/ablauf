...
await ablauf.sendEvent(WorkflowClass, {
	id: workflowId,
	event: "event-name",
	payload: { /* event data matching the schema */ },
});
```
...
<Callout type="warn">
**Note:** You can't catch `EventTimeoutError` in a try/catch like this in the actual workflow. The error is thrown across workflow replays. If a timeout occurs, the workflow will be marked as failed with the `EventTimeoutError`. Handle timeouts by checking workflow status externally or designing your workflow to avoid them.
</Callout>
