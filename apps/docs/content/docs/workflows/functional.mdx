...
A Zod schema defining the shape of SSE messages your workflow can broadcast. Use discriminated unions for multiple message types.
...
const ApprovalWorkflow = defineWorkflow({
	...
	sseUpdates: z.discriminatedUnion("type", [
		...
		z.object({ type: z.literal("complete"), approved: z.boolean() }),
	]),
	defaults: {
		retries: {
			limit: 5,
	...
	run: async (step, payload, sse) => {
		sse.broadcast({ type: "progress", message: "Processing request..." });
		...
		sse.emit({ type: "complete", approved: decision.approved });
...
The `defineWorkflow()` function is actually a convenience wrapper that creates a class extending `BaseWorkflow`. Both the functional and class-based APIs are functionally identical and compile to the same thing.
