...
```ts
const data = await step.do("call-flaky-api", async () => {
...
```ts
{ retries: { limit: 5 } } // Try up to 5 times total
```
...
```ts
{ retries: { delay: "5s" } } // Wait 5 seconds between retries
```
...
```ts
try {
	await step.do("impossible-task", async () => {
