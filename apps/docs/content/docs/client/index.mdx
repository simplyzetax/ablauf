...
`@ablauf/client` is a lightweight browser client for consuming Ablauf workflows from your frontend. It provides type-safe access to the dashboard API and real-time SSE subscriptions.

## Installation

```bash
npm install @ablauf/client
```

<Callout type="info">
The client connects to the oRPC endpoint exposed by your worker. Make sure you've [set up the dashboard API](/docs/server#setting-up-the-dashboard-api) first.
</Callout>
...
```ts
import type { InferSSEUpdates } from "@ablauf/client";
...
<Callout type="info">
For a full-featured monitoring UI, check out the [Dashboard](/docs/dashboard) — a ready-made React app for workflow observability.
</Callout>
