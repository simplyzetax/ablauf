...
```bash
npm install @ablauf/workflows hono zod
```

- **`@ablauf/workflows`** — The workflow engine
...
- **[Workflows](/docs/workflows)** — Learn about `defineWorkflow()` vs `BaseWorkflow`, typed events, and SSE
- **[Steps](/docs/steps)** — Master `step.do()` retry policies, `step.sleep()` durations, and `step.waitForEvent()` timeouts
- **[Events](/docs/events)** — Send external events to running workflows (webhooks, user actions, etc.)
- **[Observability](/docs/observability)** — Stream real-time workflow progress to your frontend with SSE

---

**Stuck?** Join our [Discord](https://discord.gg/ablauf) or open an issue on [GitHub](https://github.com/ablauf/ablauf). We're here to help.
